...
Complete list of every buildable template in this repository — 49 templates across 10 categories.

---

## Base Templates
*5 files — copy `base/` to start every new agent*
...
|---|------|---------|---------|
| 6 | `components/topics/_scaffold/TopicScaffold.topic.mcs.yml` | `OnRecognizedIntent` | **Start every new topic here** — built-in error handling, telemetry, CSAT guard |
| 7 | `components/topics/action-invoke/ActionInvoke.topic.mcs.yml` | `OnRecognizedIntent` | Calls a connector action with output validation and telemetry |
| 8 | `components/topics/auth/SignIn.topic.mcs.yml` | `OnSignIn` | Sign-in flow with `Auth.SignInStarted` / `Auth.SignInCompleted` telemetry |
| 9 | `components/topics/conversation-init/ConversationInit.topic.mcs.yml` | `OnActivity` (first message) | Loads M365 user profile into `Global.UserDisplayName` and `Global.UserCountry` |
| 10 | `components/topics/disambiguation/Disambiguation.topic.mcs.yml` | `OnSelectIntent` | Clarifies ambiguous intents, logs `Agent.DisambiguationTriggered` |
| 11 | `components/topics/escalation/Escalation.topic.mcs.yml` | `BeginDialog` | Human handoff via `TransferConversation`, logs `Agent.EscalationTriggered` |
| 12 | `components/topics/feedback/Feedback.topic.mcs.yml` | `OnRecognizedIntent` / `BeginDialog` | Thumbs → star rating → free text CSAT sequence |
| 13 | `components/topics/knowledge-search/KnowledgeSearch.topic.mcs.yml` | `OnUnknownIntent` | Generative answers from knowledge sources, logs `Knowledge.AnswerFound` / `AnswerNotFound` |
| 14 | `components/topics/out-of-scope/OutOfScope.topic.mcs.yml` | `OnRecognizedIntent` | Redirects out-of-scope queries with named resource, logs `Agent.OutOfScope` |
| 15 | `components/topics/question-branch/QuestionBranch.topic.mcs.yml` | `OnRecognizedIntent` | Collects user input and branches the conversation |
| 16 | `components/topics/remove-citations/RemoveCitations.topic.mcs.yml` | `OnGeneratedResponse` | Strips `[1][2]` citation markers from AI-generated responses |
...
| # | File | Kind | Purpose |
|---|------|------|---------|
| 17 | `components/actions/connector/connector-action.mcs.yml` | `InvokeConnectorTaskAction` | Calls any Power Platform connector operation |
| 18 | `components/actions/mcp/mcp-action.mcs.yml` | `InvokeExternalAgentTaskAction` | Calls an MCP server tool |
...
| # | File | Use case |
|---|------|---------|
| 21 | `components/adaptive-cards/confirmation-card.json` | Confirm or cancel before executing an action (required for Medium safety tier) |
| 22 | `components/adaptive-cards/status-card.json` | Display action result with status colour (success / warning / error) |
| 23 | `components/adaptive-cards/form-card.json` | Collect structured input — text, date, dropdown — in a single card |
| 24 | `components/adaptive-cards/feedback-thumbs.json` | 👍 / 👎 binary satisfaction |
| 25 | `components/adaptive-cards/feedback-rating.json` | ⭐ 1–5 star rating |
| 26 | `components/adaptive-cards/feedback-text.json` | Free text comment + issue category dropdown |
